...
sprite = pg.image.load(f'{os.path.abspath(os.getcwd())}/sprites/1.png')
...
class Elevator:
    ...
    def __init__(self, img, floorVar, liftsVar):
        ...
        self.currentFloor = 1
        self.dest = 1
        self.speed = 5  # Збільшено швидкість
        self.commands = []
    ...
    def generate_commands(self, people_on_floors, current_floor):
        floors_order = calculate_lifts_order(people_on_floors)
        ...
        for elevator in cls.instances:
            if elevator.commands:
                ...
                if elevator.currentFloor == target_floor:
                    ...
                    # Reset the highlighted floor when the elevator arrives
                ...
                if elevator.ismoving != 0:
                    ...
                    if elevator.ismoving == -1:
                        passing_floor += 1  # Adjust for moving upward
...
def draw_hud(screen, gridSys, people_on_floors):
    ...
    for floor in range(1, gridSys.y + 1):
        ...
        # Highlight the randomly chosen floor in blue until the elevator arrives
...
def calculate_lifts_order(people_on_floors):
    try:
        # if Elevator.current_floor == 1:
        #     max_floor = choice([key for key, value in people_on_floors.items() if value != 0])
        #
        # else:
        #     max_floor = max([key for key, value in people_on_floors.items() if value != 0])
        max_floor = max([key for key, value in people_on_floors.items() if value != 0])
    except:
        return [1]
    if max_floor == 0:
    ...
    if people_on_floors[1] > 0:
        rand_floor = randint(2, 12)
        if max_floor > 1:
            # print(temp / ((max_floor-1)*2)+people_on_floors[1]/rand_floor)
            efficiency.append(temp / ((max_floor-1)*2)+people_on_floors[1]/(rand_floor-1))
            return [rand_floor, max_floor, 1]
        else:
            efficiency.append(people_on_floors[1]/((rand_floor-1)*2))
            return [rand_floor, 1]
    else:
        efficiency.append(temp/((max_floor-1)*2))
        return [max_floor, 1]


# Init Pygame w/RunLoop
def run_game(floorVar, liftsVar):
    ...
    for floor in range(1, gridSys.y + 1):
        if floor == 7 or floor == 12:
            people_on_floors[floor] += 1
        elif floor == 5 or floor == 2:
            people_on_floors[floor] += 7
        elif floor == 4 or floor == 3:
            people_on_floors[floor] += 2
    ...
    while running:
        ...
        try:
            ...
            draw_hud(screen, gridSys, people_on_floors)  # Використовуйте people_on_floors
